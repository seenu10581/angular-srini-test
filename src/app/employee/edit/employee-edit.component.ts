      ...
      for(let i = 0; i < this.employeesData.length; i++){
        if(this.employeeId === this.employeesData[i].id){
          this.employeeForm.patchValue({
            name: [this.employeesData[i].name, [Validators.required, Validators.maxLength(20)]],                      
            age: [this.employeesData[i].age, [Validators.required, Validators.maxLength(3)]],                      
            salary: [this.employeesData[i].salary, [Validators.required, Validators.maxLength(10)]],                      
          });
        }
      }
    }
